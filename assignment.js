...
function feetToMile(feet){
  if(feet > 0){
    ...
    "You didn't insert a valid number! Try again later."
  }
}
...
function tinyFriend(name){
  var smallFriend = name[0];
  if(smallFriend.length == 0){
      return "It can't be empty, Please add any name!";
  }
  else{
       for(var i=0; i<name.length; i++){
          var element = name[i];
          if(element.length < smallFriend.length){
            smallFriend = element;
           }
       }
  }
